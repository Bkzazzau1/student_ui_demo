...
void main() {
  group('LiveMonitoringProfile', () {
    test('exam profile keeps strict pause behaviour', () {
      ...
      expect(
        profile.shouldPauseForEventType('camera_reconnect_timeout'),
        isTrue,
      );
      expect(
        profile.shouldPauseForEventType('audio_voice_isolation_alert'),
        isTrue,
      );
      expect(profile.shouldPauseForEventType('yolo_phone_detected'), isTrue);
      expect(
        profile.shouldPauseForEventType('gaze_head_pose_deviation'),
        isFalse,
      );
      expect(
        profile.shouldPauseForEventType('sustained_gaze_head_pose_deviation'),
        isTrue,
      );
      expect(
        profile.shouldPauseForEventType('system_monitoring_unavailable'),
        isTrue,
      );
    });

    test(
      'graded assessment profile sends issues to lecturer without pausing',
      () {
    ...
    test('standard profile does not pause attempts', () {
      ...
      expect(
        profile.shouldPauseForEventType('multiple_people_detected'),
        isFalse,
      );
    });
